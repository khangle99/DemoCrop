...
private class ColorPickerWheelInnerView: UIView {
	...
	override init(frame: CGRect) {
		...
		let hueView = ConicalGradientView()
...
//        hueView.gradient.colors = [.red, .green, .blue, .cyan, .yellow, .magenta, .orange, .purple, .brown]
		hueView.gradient.colors = Color.Component.hue.sliderTintColor(for: Color(red: 1, green: 0, blue: 0, alpha: 1)).map(\.uiColor)
//        hueView.gradient.startAngle = -(.pi / 2)
//        hueView.gradient.endAngle = (3 * .pi) / 2
		//hueView.gradient.transform = CATransform3DMakeRotation(0.5 * .pi, 0, 0, 1)
		addSubview(hueView)
